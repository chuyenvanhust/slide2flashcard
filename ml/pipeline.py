...
class FlashcardPipeline:
    ...
    def run(self, pdf_path, *args, **kwargs):
        progress_callback = kwargs.get('progress_callback', lambda pct, msg: None)

        # ----------------------------------------------------------------------
        # BƯỚC 1: QUÉT TRANG, PHÂN CỤM DBSCAN & TRÍCH XUẤT EMBEDDING TỰ ĐỘNG
        # ----------------------------------------------------------------------
        progress_callback(10, "Đang quét toàn bộ slide và phân cụm hình ảnh...")
        ...
        # ----------------------------------------------------------------------
        progress_callback(30, "Đang trích xuất ngữ cảnh văn bản xung quanh sơ đồ...")
        ...
        # ----------------------------------------------------------------------
        progress_callback(40, "Đang sinh nội dung flashcard bằng AI đa luồng...")
        ...
        batch_responses = self.generator.generate_batch(selected_items, ctx, max_workers=2, progress_callback=progress_callback)

        # ----------------------------------------------------------------------
        # BƯỚC 4: ĐÓNG GÓI DỮ LIỆU ĐẦU RA (DATA MAPPING)
        # ----------------------------------------------------------------------
        progress_callback(75, "Đang đóng gói dữ liệu và chuẩn bị lưu...")
        final_cards = []
        for res in batch_responses:
            if res["status"] != "success":
                print(f"⚠️ Cảnh báo: Bỏ qua ảnh {res.get('crop_path')} vì: {res.get('response')}")
            ...
            final_cards.append({
                ...
                "card_type": original_item.get('label', 'diagram'),
